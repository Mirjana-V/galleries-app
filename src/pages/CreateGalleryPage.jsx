    ...
    return (
        <div>
            ...
            <Form onSubmit={handleSubmit} className="mb-3">
                ...
                <Col sm={4}>
                <Form.Control  className="px-3 my-5" 
                ...
                <Col sm={4}>
               <Form.Control className="px-3 my-5" as="textarea" 
                ...
                {newImages && newImages.map((x, i) => {
                    return (
                        <div key={i}>
                            ...
                            {newImages?.length !== 1 && <Button variant="outline-dark" onClick={() => handleRemoveClick(i)}>Remove</Button>}
                            {newImages?.length !== 1 && <Button variant="outline-dark" type="button" onClick={() => changeOrder(i, "UP")}>Move Up</Button>}
                            {newImages?.length !== 1 && <Button variant="outline-dark" type="button" onClick={() => changeOrder(i, "DOWN")}>Move Down</Button>}
                            <div>
                                {newImages?.length - 1 === i  && <Button variant="outline-dark" onClick={handleAddClick}>Add picture</Button>}
                            </div>
                        </div>
                    )
                })}
                <br />
                <span>
                    <Button variant="outline-dark" type="submit">{id ? "Edit" : "Submit"}</Button>
